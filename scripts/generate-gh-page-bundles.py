#!/usr/bin/env python3
"""
Generate self-contained HTML bundles for each transcription (GitHub-only version).

This is a special version for GitHub Actions that MOVES files instead of copying them
to avoid exceeding the 14GB GitHub Pages limit. The original files are moved from
the seminars directory to the gh-pages-output directory, preventing duplication.

Each bundle includes:
- The compressed WebM/Opus audio file (moved, not copied)
- The transcript JSON file (moved, not copied)
...
def generate_bundle(transcription_path: Path, output_dir: Path, base_dir: Path) -> Tuple[str, str]:
    ...
    NOTE: This GitHub-only version MOVES files instead of copying them to save space.
    The original audio and transcript files are moved from the source location to
    the output directory, preventing duplication that would exceed the 14GB limit.
    
    Returns:
        (seminar_group, lecture_name) tuple
    """
    # Load transcript data BEFORE moving the file
    ...
    # Move audio file (instead of copying) to save space
    audio_filename = audio_path.name
    shutil.move(str(audio_path), str(bundle_dir / audio_filename))
    print(f"📦 Moved audio: {audio_path.name}")
    
    # Move transcript JSON (instead of copying) to save space
    shutil.move(str(transcription_path), str(bundle_dir / "transcript.json"))
    print(f"📦 Moved transcript: {transcription_path.name}")
...
def main():
    ...
    print("⚠️  NOTE: This script MOVES files instead of copying to save space (GitHub-only version)")
